// PayPal Integration - Graceful degradation when credentials not configured
import { Request, Response } from "express";

const { PAYPAL_CLIENT_ID, PAYPAL_CLIENT_SECRET } = process.env;

// Only initialize PayPal if credentials are available
let ordersController: any = null;
let oAuthAuthorizationController: any = null;

async function initializePayPal() {
  if (!PAYPAL_CLIENT_ID || !PAYPAL_CLIENT_SECRET) {
    console.log("PayPal credentials not configured - payment features disabled");
    return false;
  }

  try {
    const { Client, Environment, LogLevel, OAuthAuthorizationController, OrdersController } = 
...
export async function createPaypalOrder(req: Request, res: Response) {
  if (!ordersController) {
    return res.status(503).json({ error: "PayPal not configured. Please add PayPal credentials." });
  }
...
export async function loadPaypalDefault(req: Request, res: Response) {
  try {
    ...
    res.status(503).json({ error: "PayPal not configured. Please add PayPal credentials." });
  }
}
